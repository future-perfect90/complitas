...
export default function AreaCard({ areaName, answeredCount, totalCount, onSelect }: AreaCardProps) {
    const progress = totalCount > 0 ? (answeredCount / totalCount) * 100 : 0;
    const isComplete = answeredCount === totalCount;

    return (
        <Card className="rounded-xl shadow-md transition-transform hover:scale-105" onClick={() => onSelect(areaName)}>
